      ...
      <div class="al-filter-card">
        ...
        <div class="al-filter-actions">
          <div class="al-presets">
            <button @click="setPreset(today, today)" class="al-preset-btn"
              :class="{ active: fTglDari === today && fTglAkh === today }">Hari ini</button>
            <button @click="setPreset(yesterday, yesterday)" class="al-preset-btn"
              :class="{ active: fTglDari === yesterday && fTglAkh === yesterday }">Kemarin</button>
            <button @click="setPreset(week7, today)" class="al-preset-btn"
              :class="{ active: fTglDari === week7 && fTglAkh === today }">7 Hari</button>
            <button @click="setPreset(month30, today)" class="al-preset-btn"
              :class="{ active: fTglDari === month30 && fTglAkh === today }">30 Hari</button>
          </div>
      ...
      <div class="al-table-card">
              ...
              <tr v-for="log in logs.data" :key="log.id">
                ...
                <td>
                  <div class="al-user-cell">
                    <div class="al-user-av"
                      :style="`background:${rColor(log.user_role).bg}; color:${rColor(log.user_role).color}`">
                      {{ log.user_name?.charAt(0)?.toUpperCase() ?? '?' }}
                    </div>
                    <div class="al-user-info">
                      <p class="al-user-name">{{ log.user_name }}</p>
                      <span class="al-role-badge"
                        :style="`background:${rColor(log.user_role).bg}; color:${rColor(log.user_role).color}`">
                ...
                <!-- Aktivitas -->
                <td>
                  <p class="al-desc">{{ log.aktivitas }}</p>
                </td>
                ...
                <!-- Waktu -->
                <td>
                  <p class="al-time">{{ log.created_at }}</p>
                </td>
        ...
        <div v-if="logs.data?.length" class="al-table-footer">
          <p class="al-footer-info">
            Menampilkan
            <strong style="color:var(--text-primary)">{{ meta.from ?? 1 }}</strong>–<strong
              style="color:var(--text-primary)">{{ meta.to ?? logs.data.length }}</strong>
            dari
            <strong style="color:var(--text-primary)">{{ meta.total ?? logs.data.length }}</strong>
            entri
          </p>
          <div v-if="meta.last_page > 1" class="al-pagination">
            <button v-for="link in links" :key="link.label" @click="goToPage(link.url)"
              :disabled="!link.url || link.active" class="al-page-btn"
              :class="{ 'al-page-btn--active': link.active, 'al-page-btn--disabled': !link.url }" v-html="link.label">
            </button>
          </div>
        </div>
      </div>
