...
const resetFilter = () => {
  fJenis.value=''; fStatus.value=''; fAsal.value=''; fNama.value='';
...
<template>
  <AppLayout :flash="flash" page-title="Menu Yanmed">
    <div class="p-6 sm:p-8 space-y-6"
      ...
      <!-- ═══ HERO ═══════════════════════════════════════════════════════════ -->
      <div style="background:linear-gradient(135deg,#1D4ED8 0%,#1e40af 100%);border-radius:16px;padding:22px 28px;position:relative;overflow:hidden;border:1px solid rgba(255,255,255,.1);box-shadow:0 12px 32px rgba(29,78,216,.2)">
        ...
        <p style="color:rgba(255,255,255,.6);font-size:11px;font-weight:500">Yankesmas & Medis</p>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
        <div v-for="card in [
          { label:'Total Pasien',   val: summary.total,         color:'#1D4ED8', icon:'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2' },
          ...
          { label:'Terkonfirmasi',  val: summary.terkonfirmasi, color:'#059669', icon:'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
          { label:'Bed Terisi',     val: summary.bed_terisi,    color:'#DC2626', icon:'M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z' },
          { label:'Bed Kosong',     val: summary.bed_kosong,    color:'#16A34A', icon:'M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z' },
        ]" :key="card.label"
          ...
          <div class="min-w-0">
            <p class="text-xl font-black" :style="`color:${card.color};font-family:'DM Mono',monospace;line-height:1`">{{ card.val ?? 0 }}</p>
            <p class="text-xs font-semibold mt-0.5 truncate" style="color:var(--text-secondary)">{{ card.label }}</p>
          </div>
        </div>
      </div>

      <!-- ═══ TOP ASAL RUANG ═════════════════════════════════════════════════ -->
      <div v-if="summary.by_asal?.length" class="rounded-2xl p-5"
        style="background:var(--bg-surface);border:1px solid var(--border-default);box-shadow:var(--shadow-card)">
        <p class="text-xs font-bold uppercase tracking-wider mb-3" style="color:var(--text-muted)">Top Asal Rujukan / Ruang</p>
        <div class="flex flex-wrap gap-2">
          <button v-for="a in summary.by_asal" :key="a.asal"
            @click="fAsal = a.asal === 'Tidak diketahui' ? '' : a.asal; applyFilters()"
            class="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all hover:-translate-y-px"
            style="background:rgba(29,78,216,.08);border:1px solid rgba(29,78,216,.18);color:#1D4ED8">
            <span>{{ a.asal }}</span>
            <span class="px-1.5 py-0.5 rounded-full font-black text-xs"
              style="background:#1D4ED8;color:#fff">{{ a.jumlah }}</span>
          </button>
        </div>
      </div>
        ...
        <div class="flex flex-wrap items-center gap-3">
          <div class="flex gap-1 p-1 rounded-xl" style="background:var(--bg-input)">
            <button v-for="p in [{l:'Hari ini',d:today,s:today},{l:'Kemarin',d:yesterday,s:yesterday},{l:'7 Hari',d:week7,s:today}]"
              ...
              :style="fTglDari===p.d&&fTglAkh===p.s ? 'background:#fff;color:#1D4ED8;box-shadow:0 1px 4px rgba(0,0,0,.08)' : 'color:var(--text-muted)'">
              {{ p.l }}
            </button>
          </div>
          <div v-if="fAsal" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold"
            style="background:rgba(29,78,216,.1);color:#1D4ED8;border:1px solid rgba(29,78,216,.2)">
            Asal: {{ fAsal }}
            <button @click="fAsal='';applyFilters()" class="ml-1 hover:opacity-70">✕</button>
          </div>
          <button v-if="fJenis||fStatus||fNama||fAsal"
            @click="resetFilter"
      ...
      <div v-if="!pasien.length" class="rounded-2xl flex flex-col items-center justify-center py-20 text-center"
        ...
        <div class="w-16 h-16 rounded-2xl flex items-center justify-center mb-4"
          style="background:rgba(29,78,216,.08);border:1.5px dashed rgba(29,78,216,.25)">
          <svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="#1D4ED8" stroke-width="1.5">
            <path stroke-linecap="round" stroke-linejoin="round"
              d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
          </svg>
        </div>
        <p class="font-bold text-sm mb-1" style="color:var(--text-primary)">Tidak ada data ditemukan</p>
        <p class="text-xs max-w-xs" style="color:var(--text-muted)">Coba ubah filter pencarian atau rentang tanggal.</p>
      </div>
      ...
      <div v-else class="rounded-2xl overflow-hidden"
            ...
            <tbody>
              <tr v-for="(item, idx) in pasien" :key="`${item.sumber}-${item.id}`"
                ...
                <td class="px-4 py-3.5" style="max-width:200px">
                  <p class="text-sm font-semibold break-words whitespace-normal" style="color:var(--text-primary)">
                    {{ item.diagnosa ?? '—' }}
                  </p>
                  <p v-if="item.indikasi" class="text-xs mt-0.5 break-words whitespace-normal" style="color:var(--text-muted)">
                    {{ item.indikasi }}
                  </p>
                  <p v-if="item.diagnosa_icd" class="text-xs mt-0.5 font-mono"
                    style="color:#1D4ED8">ICD: {{ item.diagnosa_icd }}</p>
                </td>
                <td class="px-4 py-3.5">
                  <span v-if="item.nama_bed" class="inline-flex items-center gap-1 text-sm font-semibold" style="color:#00A884">
                    🛏 {{ item.nama_bed }}
                  </span>
                ...
                <td class="px-4 py-3.5 font-mono text-xs whitespace-nowrap" style="color:var(--text-secondary)">
                  {{ item.created_at_fmt }}
                </td>
        ...
        <div class="block md:hidden divide-y" style="border-color:var(--border-row)">
          <div v-for="(item, idx) in pasien" :key="`mob-${item.sumber}-${item.id}`"
            @click="openDetail(item)"
            ...
            <div class="flex items-start justify-between gap-2 mb-2 pr-6">
              ...
              <span class="font-mono text-xs flex-shrink-0" style="color:var(--text-muted)">{{ item.created_at_fmt?.split(' ')[0] }}</span>
            </div>
            ...
            <div class="absolute right-4 top-1/2 -translate-y-1/2">
              <svg class="w-4 h-4 transition-transform group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" style="color:var(--text-muted)"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between px-5 py-3"
          ...
          <p class="text-xs" style="color:var(--text-secondary)">
            ...
            <span v-if="summary.total_ext || summary.total_int" style="color:var(--text-muted)">
              ({{ summary.total_ext }} Ext · {{ summary.total_int }} Int)
            </span>
          </p>
        </div>
      </div>
          ...
          <div class="overflow-y-auto px-6 py-5 space-y-4 flex-1">
            <div class="grid grid-cols-2 gap-4">
              <div class="space-y-0.5">
                ...
                <p class="text-sm font-bold font-mono" style="color:var(--text-primary)">{{ detail.No_MR ?? '—' }}</p>
              </div>
              <div class="space-y-0.5">
                <p class="text-xs font-medium" style="color:var(--text-muted)">Jenis Kelamin</p>
                <p class="text-sm font-bold" :style="`color:${gColor(detail.jenis_kelamin)}`">
                  {{ gIcon(detail.jenis_kelamin) }} {{ detail.jenis_kelamin === 'L' ? 'Laki-laki' : detail.jenis_kelamin === 'P' ? 'Perempuan' : '—' }}
                </p>
              </div>
              <div class="col-span-2 space-y-0.5">
                <p class="text-xs font-medium" style="color:var(--text-muted)">Asal Ruang / Rujukan</p>
                <p class="text-sm font-bold" style="color:var(--text-primary)">{{ detail.asal ?? '—' }}</p>
              </div>
              <div class="col-span-2 space-y-0.5">
                <p class="text-xs font-medium" style="color:var(--text-muted)">Diagnosa</p>
                <p class="text-sm font-bold" style="color:var(--text-primary)">{{ detail.diagnosa ?? '—' }}</p>
                <p v-if="detail.diagnosa_icd" class="text-xs font-mono mt-0.5" style="color:#1D4ED8">ICD: {{ detail.diagnosa_icd }}</p>
              </div>
              <div class="col-span-2 space-y-0.5">
                <p class="text-xs font-medium" style="color:var(--text-muted)">Indikasi / Rencana Tindakan</p>
                <p class="text-sm font-bold" style="color:var(--text-primary)">{{ detail.indikasi ?? '—' }}</p>
              </div>
              <div v-if="detail.dokter" class="space-y-0.5">
                <p class="text-xs font-medium" style="color:var(--text-muted)">DPJP</p>
                <p class="text-sm font-bold" style="color:var(--text-primary)">{{ detail.dokter }}</p>
              </div>
              <div class="space-y-0.5">
                ...
                <p class="text-sm font-bold flex items-center gap-1.5" style="color:#00A884">
                  ...
                  <span v-else style="color:var(--text-muted)">{{ detail.kebutuhan_bed ?? '—' }}</span>
                </p>
              </div>
              <div v-if="detail.jaminan" class="space-y-0.5">
                <p class="text-xs font-medium" style="color:var(--text-muted)">Jaminan</p>
                <span class="inline-block text-xs font-semibold px-2.5 py-1 rounded-lg"
                  style="background:#D1FAF0;color:#00A884">{{ detail.jaminan }}</span>
              </div>
            </div>

            <!-- Dokter Kolab -->
            <div v-if="detail.dokter_kolab?.length" class="rounded-xl p-4"
              style="background:var(--bg-surface-2);border:1px solid var(--border-default)">
              <p class="text-xs font-bold mb-2" style="color:var(--text-muted)">Dokter Kolaborasi</p>
              <div class="space-y-1.5">
                <div v-for="d in detail.dokter_kolab" :key="d.nama"
                  ...
                  <span class="text-xs" style="color:var(--text-muted)">({{ d.ket }})</span>
                </div>
              </div>
            </div>
            ...
            <div class="rounded-xl overflow-hidden" style="border:1px solid var(--border-default)">
              <div class="px-4 py-2.5" style="background:var(--bg-surface-2);border-bottom:1px solid var(--border-default)">
                <p class="text-xs font-bold uppercase tracking-wider" style="color:var(--text-muted)">Timeline</p>
              </div>
              <div class="px-4 py-3 space-y-2.5">
                <div class="flex items-start gap-3">
                  <div class="w-2 h-2 rounded-full mt-1.5 flex-shrink-0" style="background:#1D4ED8"></div>
                  <div>
                    <p class="text-xs font-semibold" style="color:var(--text-primary)">Booking dibuat</p>
                    <p class="text-xs font-mono" style="color:var(--text-muted)">{{ detail.created_at_fmt ?? '—' }}</p>
                  </div>
                </div>
                <div v-if="detail.approved_at_fmt || detail.confirmed_at_fmt" class="flex items-start gap-3">
                  ...
                  <div>
                    <p class="text-xs font-semibold" style="color:var(--text-primary)">
                      {{ detail.sumber === 'internal' ? 'Disetujui Admisi' : 'Bed Dikonfirmasi ICU' }}
                    </p>
                    <p class="text-xs font-mono" style="color:var(--text-muted)">
                      {{ detail.approved_at_fmt ?? detail.confirmed_at_fmt ?? '—' }}
                    </p>
                  </div>
                </div>
                <div v-if="detail.verified_at_fmt" class="flex items-start gap-3">
                  ...
                  <div>
                    <p class="text-xs font-semibold" style="color:var(--text-primary)">
                      {{ detail.sumber === 'internal' ? 'Bed Terverifikasi ICU' : 'Terverifikasi Admisi' }}
                    </p>
