...
// Diagnosis dari rekam medis RS — display only di section 2, tidak di-submit
...
const doLookup = async (noMr) => {
    ...
    try {
        ...
        if (data.found) {
            ...
            // Auto-pilih jika hanya satu kunjungan
            if (kunjunganList.value.length === 1) {
                ...
                // JANGAN set form.Diagnosis — diisi manual di section 3 (ICD10 search)
            }

            // Prefill IndikasiRI dari SPRI sebelumnya (jika ada), tapi BUKAN Diagnosis
            if (data.prefill) {
...
const form = useForm({
    No_MR:      '',
    No_Reg:     '',
    Diagnosis:  '',   // ICD10 ICU — diisi section 3, bukan dari lookup
    IndikasiRI: '',
    asal_ruang: '',   // dari rekam medis, dikirim sebagai info saja
    Dokter:     '',   // dari rekam medis, dikirim sebagai info saja
    spesialis:  '',
    Keterangan: '',
});
            ...
            <Transition enter-active-class="transition-all duration-200 ease-out"
                ...
                <form v-if="showForm && canBuatSpriInternal" @submit.prevent="submitForm" class="card-dark overflow-hidden">
                    ...
                    <div class="p-5 space-y-5">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <input type="text" :value="form.No_Reg" readonly tabindex="-1"
                        ...
                        <!-- 2. Data Klinis Pasien Sebelum Rawat ICU -->
                        <div class="mt-4">
                            <p class="text-xs font-bold uppercase tracking-wide mb-3" style="color:var(--text-accent)">
                                2. Data Klinis Pasien Sebelum Rawat ICU
                            </p>
                        ...
                        <!-- 3. Data Klinis Pasien Siap Rawat ICU — diisi petugas -->
                        <div>
                            <p class="text-xs font-bold uppercase tracking-wide mb-3" style="color:var(--text-accent)">
                                3. Data Klinis Pasien Siap Rawat ICU
                            </p>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    <label class="block text-xs font-semibold mb-1" style="color:var(--text-primary)">
                                    ...
                                    <Icd10Search
                                        ...
                                        :required="true"
                                        :has-error="!!form.errors.Diagnosis"
                                    />
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <template v-if="s.status === 'pending_admisi'">
                                <template v-if="canApproveAdmisi">
                                    <button v-if="!showCatatanForm[s.id]" @click="showCatatanForm[s.id] = true"
                                        class="text-xs font-bold py-2 rounded-xl" style="background:#2DD9A4; color:#0D1A17">
                                        ✓ Setujui &amp; Isi Catatan
                                    </button>
                                    <div v-else class="space-y-2">
                                        ...
                                        <div class="flex gap-1.5">
                                            <button @click="doApprove(s)" class="flex-1 text-xs font-bold py-2 rounded-xl" style="background:#2DD9A4; color:#0D1A17">✓ Setujui</button>
                                            <button @click="showCatatanForm[s.id] = false" class="text-xs px-3 py-2 rounded-xl" style="background:var(--bg-input); color:var(--text-secondary)">Batal</button>
                                        </div>
                                    </div>
                                    <div v-if="!showTolakForm['adm_'+s.id]">
                                        <button @click="showTolakForm['adm_'+s.id] = true"
                                            ...
                                            style="background:rgba(224,112,80,0.1); color:#E07050; border:1px solid rgba(224,112,80,0.25)">✕ Tolak</button>
                                    </div>
                                    <div v-else class="space-y-1.5">
                                        ...
                                        <button @click="doTolakAdmisi(s)" class="w-full text-xs font-bold py-1.5 rounded-xl" style="background:#E07050; color:#fff">Konfirmasi Tolak</button>
                                    </div>
                                </template>
                            ...
                            <template v-else-if="s.status === 'pending_icu'">
                                <template v-if="canBookingBedIcu">
                                    ...
                                    <div>
                                        <p class="text-xs font-semibold mb-1" style="color:var(--text-primary)">1. Tentukan Jenis ICU:</p>
                                    ...
                                    <button v-if="props.kamarKosong.length > 0" @click="doBookingBed(s)"
                                        class="text-xs font-bold py-2 rounded-xl" style="background:#2DD9A4; color:#0D1A17">
                                        ✓ Alokasikan Bed
                                    </button>
                                    ...
                                    <div v-else class="space-y-1.5">
                                        <input v-model="alasanTolak['icu_'+s.id]" placeholder="Alasan *"
                                            class="w-full text-xs px-3 py-2 rounded-xl outline-none"
                                            style="border:1px solid rgba(224,112,80,0.3); background:var(--bg-surface); color:var(--text-primary)"/>
                                        <button @click="doTolakIcu(s)" class="w-full text-xs font-bold py-1.5 rounded-xl" style="background:#E07050; color:#fff">Konfirmasi Tolak</button>
                                    </div>
                                </template>
                                <span v-else class="text-xs text-center py-2 rounded-xl"
                            ...
                            <template v-else-if="s.status === 'bed_booked'">
                                ...
                                <button v-if="canKonfirmasiMasuk" @click="doKonfirmasiMasuk(s)"
                                    class="text-xs font-bold py-2.5 rounded-xl" style="background:#2DD9A4; color:#0D1A17">
