...
const theme = (bed) => {
    ...
    if (s === 'KOSONG')  return { bg: 'rgba(45,217,164,0.06)',  border: 'rgba(45,217,164,0.2)',  stripe: '#2DD9A4', badge: 'rgba(45,217,164,0.15)',  txt: '#2DD9A4',  label: 'Tersedia',  dot: '#2DD9A4',  pulse: false };
    if (s === 'BOOKING') return { bg: 'rgba(224,146,58,0.06)',  border: 'rgba(224,146,58,0.2)',  stripe: '#E0923A', badge: 'rgba(224,146,58,0.15)',  txt: '#E0923A',  label: 'Booking',   dot: '#E0923A',  pulse: false };
    return                      { bg: 'rgba(74,144,217,0.06)',  border: 'rgba(74,144,217,0.2)',  stripe: '#4A90D9', badge: 'rgba(74,144,217,0.15)',  txt: '#4A90D9',  label: 'Terisi',    dot: '#4A90D9',  pulse: true  };
};
...
<template>
    <AppLayout :flash="flash" page-title="Informasi Bed ICU">
        <div class="p-4 sm:p-6 space-y-5" style="font-family:'Plus Jakarta Sans',sans-serif">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <button @click="filterStatus = 'semua'"
                    ...
                    :style="filterStatus === 'semua' ? 'border-color:rgba(142,168,158,0.5); background:rgba(142,168,158,0.06)' : ''">
                ...
                <button @click="filterStatus = filterStatus === 'KOSONG' ? 'semua' : 'KOSONG'"
                    class="card-dark p-4 text-left transition-all"
                    :style="filterStatus === 'KOSONG' ? 'border-color:rgba(45,217,164,0.6); background:rgba(45,217,164,0.06)' : ''">
                    <p class="text-xs font-semibold mb-1" style="color:#2DD9A4">Tersedia</p>
                    <p class="text-3xl font-bold" style="color:#2DD9A4; font-family:'DM Mono',monospace">{{ kosong }}</p>
                ...
                <button @click="filterStatus = filterStatus === 'ISI' ? 'semua' : 'ISI'"
                    class="card-dark p-4 text-left transition-all"
                    :style="filterStatus === 'ISI' ? 'border-color:rgba(74,144,217,0.6); background:rgba(74,144,217,0.06)' : ''">
                    <p class="text-xs font-semibold mb-1" style="color:#4A90D9">Terisi</p>
                    <p class="text-3xl font-bold" style="color:#4A90D9; font-family:'DM Mono',monospace">{{ terisi }}</p>
                    <p class="text-xs mt-1" style="color:var(--text-secondary)">Klik untuk filter</p>
                </button>

                <button @click="filterStatus = filterStatus === 'BOOKING' ? 'semua' : 'BOOKING'"
                    class="card-dark p-4 text-left transition-all"
                    :style="filterStatus === 'BOOKING' ? 'border-color:rgba(224,146,58,0.6); background:rgba(224,146,58,0.06)' : ''">
                    <p class="text-xs font-semibold mb-1" style="color:#E0923A">Booking</p>
                    <p class="text-3xl font-bold" style="color:#E0923A; font-family:'DM Mono',monospace">{{ booking }}</p>
                    <p class="text-xs mt-1" style="color:var(--text-secondary)">Klik untuk filter</p>
                </button>
            </div>
            ...
            <div class="card-dark p-4 space-y-2">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-sm font-bold font-mono" :style="`color:${pct > 80 ? '#E07050' : pct > 50 ? '#E0923A' : '#2DD9A4'}`">{{ pct }}%</span>
                </div>
                <div class="flex h-2.5 rounded-full overflow-hidden gap-px" style="background:rgba(255,255,255,0.06)">
                    <div class="rounded-full transition-all duration-700" style="background:#2DD9A4"
                        :style="`width:${total > 0 ? kosong/total*100 : 0}%`"></div>
                    <div class="transition-all duration-700" style="background:#E0923A"
                        :style="`width:${total > 0 ? booking/total*100 : 0}%`"></div>
                    <div class="rounded-full transition-all duration-700" style="background:#4A90D9"
                ...
                <div class="flex flex-wrap gap-3">
                    <span class="flex items-center gap-1.5 text-xs" style="color:var(--text-secondary)">
                        <span class="w-2 h-2 rounded-full" style="background:#2DD9A4"></span>{{ kosong }} Tersedia
                    </span>
                    <span class="flex items-center gap-1.5 text-xs" style="color:var(--text-secondary)">
                        <span class="w-2 h-2 rounded-full" style="background:#E0923A"></span>{{ booking }} Booking
                    </span>
                    <span class="flex items-center gap-1.5 text-xs" style="color:var(--text-secondary)">
                        <span class="w-2 h-2 rounded-full" style="background:#4A90D9"></span>{{ terisi }} Terisi
                    </span>
                </div>
            </div>
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <button @click="filterKelas = 'semua'"
                        ...
                        :style="filterKelas === 'semua'
                            ? 'background:rgba(45,217,164,0.2); color:#2DD9A4; border:1px solid rgba(45,217,164,0.4)'
                    ...
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <div class="flex items-center gap-1.5 text-xs">
                            <span class="px-2 py-0.5 rounded-full font-bold"
                                style="background:rgba(45,217,164,0.15); color:#2DD9A4; font-family:'DM Mono',monospace">
