...
const semuaKamarTerisiL = computed(() =>
  props.semuaKamar.filter(k => k.Status === 'ISI' && (k.jenis_kelamin ?? '').toUpperCase() === 'L').length
);
const semuaKamarTerisiP = computed(() =>
  props.semuaKamar.filter(k => k.Status === 'ISI' && (k.jenis_kelamin ?? '').toUpperCase() === 'P').length
);
...
const bedTheme = (bed) => {
  ...
  if (s === 'KOSONG')
    return { stripe: '#10B981', badgeBg: '#D1FAE5', badgeColor: '#065F46', label: 'Tersedia', dot: '#10B981', cardBorder: 'rgba(16,185,129,0.3)', cardBg: '#F0FDF4', genderIcon: null }

  if (s === 'BOOKING')
    return { stripe: '#F59E0B', badgeBg: '#FEF3C7', badgeColor: '#D97706', label: 'Booking', dot: '#F59E0B', cardBorder: 'rgba(245,158,11,0.3)', cardBg: '#FFFBEB', genderIcon: null }

  // ISI — warna berdasarkan jenis kelamin
  if (jk === 'L')
    return { stripe: '#3B82F6', badgeBg: '#DBEAFE', badgeColor: '#1D4ED8', label: 'Terisi (♂)', dot: '#3B82F6', cardBorder: 'rgba(59,130,246,0.35)', cardBg: '#EFF6FF', genderIcon: '♂' }

  if (jk === 'P')
    return { stripe: '#EC4899', badgeBg: '#FCE7F3', badgeColor: '#9D174D', label: 'Terisi (♀)', dot: '#EC4899', cardBorder: 'rgba(236,72,153,0.35)', cardBg: '#FDF2F8', genderIcon: '♀' }

  // ISI tapi JK tidak diketahui
  return { stripe: '#6366F1', badgeBg: '#E0E7FF', badgeColor: '#3730A3', label: 'Terisi', dot: '#6366F1', cardBorder: 'rgba(99,102,241,0.3)', cardBg: '#EEF2FF', genderIcon: '●' }
};
...
const kpiCards = computed(() => [
  ...
  { label: 'Tersedia', val: kosong.value, filter: 'KOSONG', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', iconBg: '#D1FAE5', iconColor: '#059669' },
  { label: 'Laki-laki', val: semuaKamarTerisiL.value, filter: 'ISI', icon: 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z', iconBg: '#DBEAFE', iconColor: '#1D4ED8' },
  { label: 'Perempuan', val: semuaKamarTerisiP.value, filter: 'ISI', icon: 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z', iconBg: '#FCE7F3', iconColor: '#9D174D' },
  ...
  <AppLayout :flash="flash" page-title="Informasi Bed ICU">
    <div class="denah-wrap">
      ...
      <div class="denah-page-header">
        <div class="denah-page-header-inner">
          ...
          <div class="denah-header-live">
            <span class="ping-dot"
              style="width:7px;height:7px;border-radius:50%;background:#10B981;display:inline-block"></span>
      ...
      <div class="denah-kpi-grid">
        <div v-for="c in kpiCards" :key="c.label" @click="filterStatus = filterStatus === c.filter ? 'semua' : c.filter"
          class="denah-kpi-card kpi-card" :class="filterStatus === c.filter ? 'denah-kpi-active' : ''"
          ...
          <div class="flex justify-between items-center mb-2">
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs font-semibold px-2 py-0.5 rounded-full"
                style="background:var(--bg-input);color:var(--text-muted)">{{ total }} total bed</span>
            </div>
            <span class="text-sm font-bold" style="font-family:'DM Mono',monospace"
              :style="`color:${pct > 80 ? '#DC2626' : pct > 50 ? '#D97706' : '#059669'}`">{{ pct }}% terisi</span>
          </div>
          <div class="denah-occ-bar">
            <div class="denah-occ-seg"
              :style="`width:${total > 0 ? (kosong / total * 100).toFixed(0) + '%' : '0%'};background:#10B981`"></div>
            <div class="denah-occ-seg"
              :style="`width:${total > 0 ? (booking / total * 100).toFixed(0) + '%' : '0%'};background:#F59E0B`"></div>
            <div class="denah-occ-seg"
              :style="`width:${total > 0 ? (semuaKamarTerisiL / total * 100).toFixed(0) + '%' : '0%'};background:#3B82F6`"></div>
            <div class="denah-occ-seg"
              :style="`width:${total > 0 ? (semuaKamarTerisiP / total * 100).toFixed(0) + '%' : '0%'};background:#EC4899;border-radius:0 99px 99px 0`">
            </div>
          </div>
          <div class="flex gap-4 mt-2 flex-wrap">
            <span v-for="leg in [
              { c: '#10B981', l: 'Tersedia', v: kosong },
              { c: '#F59E0B', l: 'Booking', v: booking },
              { c: '#3B82F6', l: 'Terisi (♂)', v: semuaKamarTerisiL },
              { c: '#EC4899', l: 'Terisi (♀)', v: semuaKamarTerisiP }
            ]" :key="leg.l" class="flex items-center gap-1.5 text-xs" style="color:var(--text-muted)">
        ...
        <div class="denah-filter-card">
          <div class="flex gap-3 items-center flex-wrap">
            ...
            <div class="relative flex-1 min-w-0" style="min-width:200px;max-width:400px">
              <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style="color:#94A3B8" fill="none"
              ...
              <input v-model="searchBed" placeholder="Cari nama ruang, kode, No. MR..."
                ...
                @focus="$event.target.style.borderColor = '#00A884'"
            ...
            <div class="flex gap-2 flex-wrap items-center">
              ...
              <button @click="filterKelas = 'semua'" class="denah-chip"
                :class="filterKelas === 'semua' ? 'active' : ''">Semua</button>
            ...
            <button v-if="filterStatus !== 'semua' || filterKelas !== 'semua' || searchBed"
              @click="filterStatus = 'semua'; filterKelas = 'semua'; searchBed = ''" class="denah-reset-btn">✕ Reset</button>
        ...
        <div v-for="group in grouped" :key="group.nama" class="denah-group">
          ...
          <div class="denah-bed-grid">
            <div v-for="bed in group.beds" :key="bed.Kode_Ruang" class="denah-bed-card"
              :style="`border-color:${bedTheme(bed).cardBorder};background:${bedTheme(bed).cardBg}`"
              ...
              <div class="denah-bed-body">
                ...
                <div class="flex items-start justify-between gap-2 mb-2">
                  <div class="min-w-0">
                    ...
                    <p class="text-xs mt-0.5"
                      style="color:var(--text-muted);font-family:'DM Mono',monospace;font-size:10px">{{ bed.Kode_Ruang
                      }}</p>
                  </div>
                  <span class="denah-bed-dot flex-shrink-0" :style="`background:${bedTheme(bed).dot}`"></span>
                </div>
                <!-- Status badge with gender icon -->
                <div class="flex items-center gap-1 flex-wrap">
                  <span v-if="bedTheme(bed).genderIcon" class="denah-gender-icon"
                    :style="`color:${bedTheme(bed).badgeColor}`">
                    {{ bedTheme(bed).genderIcon }}
                  </span>
                  <span class="denah-bed-badge"
                ...
                <div v-if="bed.No_MR" class="denah-bed-patient">
                  <p class="text-xs" style="color:var(--text-muted);font-family:'DM Mono',monospace">MR: {{ bed.No_MR }}
                  </p>
                  <p v-if="bed.nama_pasien" class="text-xs font-semibold mt-1 truncate"
                    style="color:var(--text-primary)">{{ bed.nama_pasien }}</p>
                  <p v-if="bed.diagnosa" class="text-xs mt-0.5 truncate" style="color:var(--text-secondary)">{{
                    bed.diagnosa }}</p>
                  <p v-if="bed.asal_ruang" class="text-xs mt-0.5 truncate" style="color:var(--text-muted)">← {{
                    bed.asal_ruang }}</p>
                </div>
                <!-- Empty bed indicator -->
                <div v-else class="denah-bed-empty-indicator">
                  <svg class="w-5 h-5" style="color:var(--text-muted);opacity:0.4" fill="none" viewBox="0 0 24 24"
...
.denah-group-accent {
  ...
  height: 40px;
  border-radius: 4px;
  background: linear-gradient(180deg, #00A884, #007a61);
  flex-shrink: 0;
}
...
.denah-group-badge.available {
  background: #ECFDF5;
  color: #059669;
}

.denah-group-badge.total {
  background: var(--bg-input);
  color: var(--text-muted);
}
